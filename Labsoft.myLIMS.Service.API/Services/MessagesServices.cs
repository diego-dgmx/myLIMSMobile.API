...
namespace Services {
    public class MessagesServices(HttpClient httpClient, IOptions<ApiSettings> settings) : IMessagesServices
    {
        ...
        public async Task<ExternalResponse<dynamic, ErrorResponse>> UpdateActiveStatus(UpdateMessageActiveStatusDTO body)
        {
            try
            {
                _httpClient.DefaultRequestHeaders.Add("x-access-key", _settings.MyLIMSApiAccessKey);
                ...
                var response = await _httpClient.PutAsync($"{_settings.MyLIMSApiURLBase}/MessageApi/UpdateActiveStatus", content);
